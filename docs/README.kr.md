...
- `codex login status`? `codex.ps1` ?? ?? ??? ???? `codex.cmd login status`? ???? ???.
- Windows bot/tray? ???? Codex ?? ??(`codex.cmd`, `codex.exe`, `codex`)? ?? ???? `~/.codex/codex-discord-runtime.json`? ?????.
...
- Windows PowerShell?? `codex login status`? `codex.ps1` ?? ?? ??? ???? `codex.cmd login status`? ???? ???.
- Windows bot/tray? ???? Codex ?? ??(`codex.cmd`, `codex.exe`, `codex`)? ?? ???? `~/.codex/codex-discord-runtime.json`? ?????.
- Linux / Windows ???? ????? ?? Codex ??? ??? ??? ? ????.
- Windows tray ????? ??, ?? ??, Codex ???? ?? ?????.
